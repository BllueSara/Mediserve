...
function loadTicketTypes() {
  fetch("http://localhost:5050/ticket-types")
    ...
    .then(types => {
      ...
      addNewRow.innerHTML = `<div class="dropdown-option-text">+ ${t['add_new_ticket_type']}</div>`;
      addNewRow.onclick = () => {
        sessionStorage.setItem("lastDropdownOpened", "ticket-type");
        openGenericPopup(t['ticket_type'], "ticket-type");
        closeAllDropdowns();
      };
...
function loadReportStatuses() {
  fetch("http://localhost:5050/report-statuses")
    ...
    .then(statuses => {
      ...
      const addNewRow = document.createElement("div");
      addNewRow.className = "dropdown-option-row add-new-option";
      addNewRow.innerHTML = `<div class="dropdown-option-text">+ ${t['add_new_report_status']}</div>`;
      addNewRow.onclick = () => {
        sessionStorage.setItem("lastDropdownOpened", "report-status");
        openGenericPopup(t['report_status'], "report-status");
        closeAllDropdowns();
      };
      optionsContainer.appendChild(addNewRow);
